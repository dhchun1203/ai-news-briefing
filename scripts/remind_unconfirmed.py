...
# 가입 후 이만큼 지나도 확인이 없으면 리마인더 대상.
#
# **이 숫자는 실제 대기 시간이 아니다.** 이 스크립트는 매일 08:00 KST 파이프라인에서
# 한 번만 도므로, 가입 시각이 그 시각보다 뒤면 그날 실행은 놓치고 다음 날로 밀린다.
# 즉 실제 대기는 N일이 아니라 **N~N+1일**이다.
#
# 2로 뒀을 때 실측: 08:19 KST에 가입한 사람이 47.7시간 시점의 실행에서 20분 차이로
# 걸러지고 결국 71.7시간을 기다렸다. 확인 메일을 놓친 사람에게 사흘 뒤에 보내는 건
# 늦다 — 그 사이 관심이 식는다.
#
# 1이면 실제 대기가 24~48시간이 되어 "이틀 안팎"이라는 원래 의도에 맞는다. 최소 24시간은
# 확보되므로 "지금 메일함 보고 있는데 재촉당하는" 경우도 생기지 않는다.
