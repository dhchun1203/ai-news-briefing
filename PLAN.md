...
- `site-desktop.css`: `media="(min-width: 768px)"`로 조건부 적용. 본문 폭을 720px로 제한해
  한 줄이 너무 길어지지 않게 하고, 카드에 hover 반응 추가 (§20에서 760px→720px로
  조정하면서 `.header-inner`와 `.content`를 같은 폭으로 통일했다)
...
**같은 사건, 다른 링크 중복 방지**: `cross_source_count`는 순위만 올릴 뿐 실제 선택
개수를 제한하지 않아서, 같은 사건을 다루는 서로 다른 출처의 기사 여러 개가 top-10
자리를 나눠 차지하는 문제가 있었다(링크는 다르니 `load_published_links()`로도 못
거름 — 사용자가 "같은 기사는 아니어도 내용이 겹치는 기사가 보인다"고 지적). 이를
막기 위해 `build_same_story_clusters()`가 별도로 추가됐다(union-find). 처음엔 "제목
키워드가 절반 이상 겹치면 같은 사건"이라는 비율 기준으로 짰는데, 테스트해보니 제목이
짧아 키워드가 2개뿐인 기사("OpenAI unveils GPT-6")가 있으면 회사명 "OpenAI" 하나만
겹쳐도 50%를 넘어버려, 완전히 무관한 "OpenAI 소송" 기사까지 GPT-6 클러스터에
잘못 합쳐지는 오탐이 실제로 발생했다(직접 합성 테스트 케이스로 확인). 그래서 그날
후보 전체에서 각 키워드가 몇 개의 서로 다른 제목에 등장하는지(document frequency)
먼저 세어, 6개 초과 제목에 등장하는 흔한 키워드(대개 회사명)는 매칭에서 제외하고,
남은 소수의 제목에서만 등장하는 "사건을 특정하는" 키워드(제품명·버전 등, 예:
"GPT-6", "AlphaFold")가 하나라도 겹치면 같은 사건으로 묶는 방식으로 교체했다 —
합성 테스트로 재확인(같은 사건 3개는 합쳐지고, 회사명만 겹치는 무관한 기사·전혀
다른 사건은 분리됨). 이후 실제 RSS 피드로 재검증하다가, 문턱값을 3으로 뒀을 때
"Anthropic"(그날 8개 제목에 등장)뿐 아니라 "Opus"(5개 제목에 등장)까지 "흔한
키워드"로 제외돼, 정작 5개 매체가 동시에 다룬 진짜 같은 사건(Claude Opus 5 출시)을
못 묶는 반대 방향 문제가 실측 데이터로 발견됐다 — "여러 매체가 동시에 다룸"은
바로 이 로직이 잡아야 하는 신호 그 자체였는데 문턱값이 너무 낮아 그 신호까지
같이 걸러낸 것이었다. 실제 그날 후보(112개)의 키워드 등장 빈도를 직접 집계해
"OpenAI"(13)·"Anthropic"(8)·"ChatGPT"(7)는 걸러지고 "Opus"(5)·"Claude"(4)는
남도록 문턱값을 6으로 보정했다. 동시에 "Meet the New Claude Opus 5" 같은 제목의
"Meet"처럼 헤드라인 관용구라 대문자로 시작할 뿐 실제 키워드가 아닌 단어들이
새로 눈에 띄어 `TITLE_STOPWORDS`에 추가했다. `compute_cross_source_counts()`
(순위 매기기용, 느슨한 기준)와는 다른 별도 기준이다. top-10을 채울 때 클러스터당 하나만 채택하고, 부족분을 채우는
단계도 1차로는 출처 다양성 제한만 풀고 클러스터 중복 방지는 유지, 그래도
부족하면(사건 종류 자체가 10개보다 적은 날) 최후 수단으로 클러스터 제한까지 푼다.
...
### 23. 용어사전 연결 그래프 ("용어 지도") — 2026-07-27 제거됨
아래는 구현 당시 기록이다. 용어가 실제로 쌓이면서(16개 이상) 라벨이 겹치고 잘리는
등 밀도 문제가 §7 적응형 크기 조정만으로는 충분히 해소되지 않았고, 사용자가 "그래프는
삭제하자"고 판단해 전체를 걷어냈다 — HTML/CSS/JS(`templates/glossary.html.j2`,
`templates/static/site.js`, CSS 3파일의 `.glossary-graph*`/`.glossary-node`),
`generate_site.py`의 `load_glossary_relations()`/`graph_data` 계산, SKILL.md의
관계 판단 단계, `docs/glossary-relations.json` 전부 제거. 용어사전은 다시 순수
텍스트 목록(`<dl>`)만 남았다. 나중에 재시도한다면 이번 시행착오(부유 애니메이션
수렴 처리, 렌더 좌표 clamp, 적응형 크기)를 참고할 것 — 근본 원인은 애니메이션
버그가 아니라 **용어 수가 늘어날수록 평면 그래프 자체의 정보 밀도가 감당이 안 되는
구조적 문제**였다는 점이 핵심 교훈이다.
...
부유 애니메이션은 처음엔 매 프레임 무작위 속도를 더한 뒤 감쇠하는 방식이었는데,
반발력·스프링·중앙정렬력이 지배적이라 평형에 도달하면 흔들림이 죽어 눈에 안 보이는
문제가 있었다 — 힘 시뮬레이션(레이아웃 결정)과 렌더링 좌표를 분리해, 평형 위치 위에
노드별로 다른 주기/위상/진폭의 사인파를 항상 더하는 방식으로 교체했다(그래프 구조는
그대로 두고 화면에 그리는 좌표만 계속 움직임). 이 사인파 오프셋이 가장자리 노드를
컨테이너 밖으로 밀어낼 수 있어(폭이 좁은 모바일에서 특히), 렌더 좌표도 물리 좌표와
같은 여백(margin, 컨테이너 크기에 비례해 계산)으로 다시 한번 clamp한다.

용어가 늘어나며 그래프가 빽빽해지는 문제는 두 가지로 완화: (1) 엣지가 하나도 없는
용어는 그래프에서 아예 제외(연결 없는 점은 "관계 지도"에 기여하는 바가 없고 자리만
차지함 — 아래 전체 목록에는 그대로 남아 정보 손실 없음), (2) 연결된 용어 수가
늘어날수록 그래프 영역을 상한선까지 조금씩 넓힌다(높이는 데스크톱/모바일 둘 다,
가로 폭은 본문 컨테이너 720px를 넘어서는 "브레이크아웃"이라 가로 여백이 넉넉한
데스크톱에서만 — `left:50%; transform:translateX(-50%); width:min(var(--graph-width),
calc(100vw - 32px))`로 뷰포트 초과는 방지). 크기는 `generate_site.py`가 연결된
노드 수 기준으로 계산해 인라인 CSS 커스텀 프로퍼티(`--graph-width`,
`--graph-height-desktop`, `--graph-height-mobile`)로 넘긴다.

### 24. 전면 점검 (2026-07-26) — 신뢰성·보안·접근성
운영 데이터가 며칠 쌓인 뒤 코드베이스 전체(스크립트 7개, 템플릿 3개+CSS 3개, 서버리스
함수 6개, 설정·문서)를 감사했다. 발견의 상당수가 **조용히 실패해서 아무도 모르는**
부류였다 — 실패 알림도, 로그도, 눈에 보이는 증상도 없이 기능이 죽어 있었다.

**긴급 2건**
- **주간 회고가 매주 건너뛰어지고 있었다.** Routine은 `0 23 * * *`(UTC)로 도는데
  SKILL.md 5단계가 셸 `date +%u`로 요일을 봤다. 08:00 KST 일요일 시점에 컨테이너는
  아직 토요일이라 `%u`가 6을 반환 → 조건이 영원히 거짓. 첫 일요일(2026-07-26) 실행
  직전에 발견했다. `docs/weekly/`가 한 번도 생긴 적이 없다는 게 증거였다.
  `TZ=Asia/Seoul date`로 고치려다 **더 나쁜 함정**을 발견해 방향을 바꿨다 —
  `TZ=<지역명>`은 시스템 IANA tzdata가 있어야 하고 없으면 **에러 없이 조용히 UTC로
  폴백**한다(개발 환경인 Windows Git Bash가 실제로 그렇다). 고쳤다고 착각한 채 같은
  버그가 남을 수 있어, `requirements.txt`에 고정된 `tzdata`를 쓰는 Python
  `zoneinfo` 기반 `scripts/kst_date.py`로 대체했다.
- **공개 저장소 README에 실사용 HMAC 시크릿이 커밋돼 있었다**(`.env` 값과 해시 일치로
  확인, 저장소는 `private: false`). 이 키로 구독 확인 토큰을 위조해 임의 주소를 동의
  없이 구독자로 만들거나, 만료 없는 취소 토큰으로 기존 구독자를 해지시킬 수 있었다.
  교체 후 제거하고 생성 명령만 안내하도록 바꿨다.

**발송 신뢰성** — 메일 HTML에서 기사 제목·요약은 이스케이프하면서 바로 옆
`a["link"]`만 raw로 `href`에 넣고 있었다(서드파티 RSS 값이라 실제로 속성 인젝션이
가능했고 재현도 확인). 배치 중간 실패 시 마커를 안 남겨 catch-up이 **이미 받은
사람에게 재발송**할 수 있었다(→ partial 마커). `urlopen` 타임아웃 부재로 무인 실행이
무한 대기할 수 있었고, Resend 초당 한도에 걸리는 429가 재시도 대상이 아니었다.
`.sent.json`이 배포(6단계) 뒤에 생성돼 **한 번도 커밋된 적이 없었다** — catch-up의
유일한 영속 상태가 영속되지 않았다(→ 7-3단계 신설).

**보안** — `/api/subscribe`에 레이트 리밋이 전무해(저장소 전체에 throttle 코드 0건)
누구든 남의 주소로 확인 메일을 무제한 발사할 수 있는 **메일 폭탄 증폭기**였다
(→ `last_confirm_sent_at` 쿨다운). 에러 응답에 Supabase 호스트·테이블명이 실려
나갔고, `SUBSCRIBE_TOKEN_SECRET` 미검증으로 "행은 쓰였는데 메일은 못 가고 스택
트레이스가 응답되는" 경로가 있었다. 구독취소가 GET이라 **메일 보안 스캐너가 사용자를
대신 해지**시키고 있었다(→ 확인 페이지 + POST). 구독 확인은 반대로 GET을 유지했다 —
사용자가 직접 시작한 절차의 마무리라 스캐너가 눌러도 피해가 없고 되돌리기도 쉽다.

**크롤링** — `robots.txt`의 `Disallow`가 마지막 그룹 뒤 빈 줄 **다음에** 놓여
규격상 어느 `User-agent`에도 속하지 않아 통째로 무시되고 있었다(원본 JSON 색인 금지가
완전히 무효).

**프론트엔드** — 3개 템플릿에 클라이언트 JS가 복붙돼 있었고 이미 드리프트된 상태였다
(weekly의 언어 전환에 `updatePlaceholders()` 누락, 용어 패널 닫기 구현 상이, 같은
데이터에 id 2개, `getCookie` 6벌). 공용 `templates/static/site.js`로 추출하고 페이지별
설정은 `<html data-*>`로 넘긴다. 자산 복사 루프도 두 생성기에 중복돼 있어
`seo_utils.copy_shared_assets()`로 합쳤다.

**접근성** — 닫힌 용어 패널이 `transform`으로만 숨겨져 **보이지 않는 닫기 버튼이 탭
포커스를 먹고** 있었고 그게 `aria-hidden` 안이라 ARIA 위반이었다(→ `inert` +
`visibility`). "복사되었습니다"가 DOM에 미리 있고 클래스만 토글돼 `aria-live`가
발화하지 않았다(→ 빈 채로 시작해 복사 시점에 채움). CSS에 `prefers-reduced-motion`
대응이 0건이었다. `aria-label`이 전부 한국어 고정이라 영어 모드 스크린리더 사용자가
한국어를 들었다(→ `data-aria-ko/en`).

**CSS 버그** — 다크모드 토글 썸이 트랙 밖으로 2px 삐져나오고 있었다(계산: 데스크톱
52-보더2-패딩6-썸20 = 최대 24px인데 26px 선언). 모바일에서 `.block-label`(h3) 마진이
초기화되지 않아 UA 기본 마진이 살아남았다. 용어 그래프 노드가 위치와 확대를 한
`transform`에 담고 거기 `transition`이 걸려 있어, JS가 매 프레임 바꾸는 위치를
브라우저가 계속 보간하며 부유 움직임을 뭉갰다(→ `translate`/`scale` 개별 속성 분리).

**지속가능성** — `search-index.json`이 상한 없이 자라 브라우저가 통째로 받아갔다
(하루 ~14KB, 1년이면 5MB → 최근 180일 제한). 매일 Claude가 손으로 쓰는 digest에
검증이 전혀 없어 오타 하나에 KeyError 스택 트레이스만 남았다(→ `load_digest()`;
이걸 짜다가 `published_at`이 사실상 필수라는 것도 발견). feedparser에 타임아웃과
커스텀 UA가 없었고, 응답은 정상인데 최근 기사가 없는 "죽은 피드"가 실패로 잡히지
않아 매일 조용히 0건만 기여했다(→ `feeds_stale` 보고; VentureBeat AI가 실제로 검출).

**테스트/CI 도입** — 그전까지 자동 검증이 **전무했다**(테스트 파일 0건, `.github/`
없음). `tests/`에 Python 단위 테스트(25건), API 핸들러 테스트(16건), Python↔Node
HMAC 동등성 테스트, jsdom 브라우저 스모크 테스트(108건)를 추가하고 GitHub Actions로
묶었다. 특히 HMAC 동등성은 **파이썬이 서명하고 Node가 검증하는** 언어를 넘는 계약인데
검증이 없어, 어긋나면 모든 구독취소 링크가 한꺼번에 조용히 죽는 구조였다. pytest 대신
표준 `unittest`를 쓴 건 이 저장소의 무의존성 기조를 유지하기 위해서다.

**손대지 않기로 한 것**: `--rule` 변수는 값이 `--text`와 같지만 "괘선"이라는 의미를
담은 별칭이라 유지했다(10곳 기계적 치환 위험 대비 사용자 이득 없음). 웹폰트 자체
호스팅도 보류했다 — 저장소의 23MB `NotoSerifKR-Variable.ttf`는 **삭제 불가**한
`og_image.py`용 렌더링 원본이고, 웹으로 내보내려면 별도 서브셋팅 도입이 필요하다.
대신 `size-adjust` 폴백으로 폰트 교체 시 레이아웃 밀림(CLS)만 잡았다.

### 25. 용어사전 연결 그래프 완전 제거 (2026-07-27)
§23의 "용어 지도"를 사용자 판단으로 전면 삭제. 실제로 용어가 16개 이상 쌓이자
라벨이 겹치고 잘리는 등, §7에서 넣었던 적응형 크기 조정(연결된 용어 수에 비례해
그래프 영역을 넓히는 방식)만으로는 밀도 문제가 해소되지 않았다. HTML/CSS/JS
(`templates/glossary.html.j2`, `templates/static/site.js`, CSS 3파일의
`.glossary-graph*`/`.glossary-node`), `generate_site.py`의
`load_glossary_relations()`/`graph_data` 계산, SKILL.md의 관계 판단 단계,
`docs/glossary-relations.json`을 전부 제거했다 — 용어사전은 다시 순수 텍스트
목록(`<dl>`)만 남았다. 상세 회고는 §23 상단에 남겨뒀다.

### 26. 기사 선별 신선도 — 룩백을 12일에서 1일로 축소 (2026-07-27)
2026-07-26자 브리핑에 2026-07-16(열흘 전) 기사가 섞여 나온 걸 사용자가 발견했다.
원인: `cross_source_count`(여러 매체가 동시에 다루는 사건 우선) 정렬이 1차 기준이라,
룩백 12일 안에서 오래된 기사라도 cross_source_count가 높으면 최신 기사보다 위로
올라올 수 있었다 — §12의 클러스터링/우선순위 로직 자체는 정상 동작했지만, 애초에
"열흘 전 기사가 후보 풀에 남아있어도 되는가"라는 상위 전제가 잘못돼 있었다.

이 파이프라인은 정확히 매일 같은 시각(08:00 KST)에 도는 일간 주기이므로, 룩백을
**1일**로 줄이면 "지난 실행 이후 새로 나온 기사 전부"와 정확히 일치해 신선도
문제가 구조적으로 재발하지 않는다(`fetch_articles.py`의 `--lookback-days` 기본값과
`SKILL.md`/`README.md`의 호출 예시를 모두 1로 변경). 실제 피드로 재검증: 후보
28개 중 10개 선별, 전부 실행 당일~하루 전 기사였다.

**부작용 하나 발견해서 같이 고쳤다**: §24에서 추가한 "죽은 피드" 감지가 후보용
`cutoff`를 그대로 재사용하고 있어서, 룩백을 1일로 줄이자마자 며칠에 한 번
발행하는 정상 피드(Google AI Blog, MIT Technology Review 등)까지 매일 "죽었다"고
오탐하기 시작했다. 죽은 피드 판단 기준을 `STALE_FEED_THRESHOLD_DAYS = 14`로
분리해 후보 선정용 룩백과 독립적으로 유지했다 — 하나는 "오늘 후보로 쓸 만큼
신선한가"(짧게), 다른 하나는 "이 매체가 사실상 발행을 멈췄는가"(길게) 판단이라
애초에 같은 값을 공유할 이유가 없었다.

부족한 날(신선한 후보가 10개가 안 되는 날)에는 억지로 오래된 기사를 채우지 않고
그보다 적은 수로 나가도록 뒀다 — `send_broadcast.py`의 메일 제목이 이미
`len(articles)`를 동적으로 반영하므로 코드 변경 없이 자연스럽게 처리된다.

### 27. 용어 슬러그 정규화 — 미착수 백로그 (2026-08-07 등록)

용어 개별 페이지의 URL은 `term_en`을 슬러그화한 값이다. 그런데 `term_en`은 매일
브리핑을 쓰면서 생성되는 자유 문자열이라, 같은 개념을 날마다 조금씩 다르게 적으면
그대로 새 URL이 된다. 실제로 오픈웨이트 하나가 `open-weight` / `open-weights` /
`open-weight-model` / `open-weight-models` 4개로 갈라졌고, 그중 둘은 용어사전이
링크하지 않는 고아가 돼 sitemap에만 살아남았다(→ 위 정리는 배포 완료).

지금 막아둔 것은 **사후 수습**이다 — `config/glossary_aliases.json`에 사람이 손으로
변형을 등록하면 접힌다. 등록을 잊으면 그날 또 갈라진다. 필요한 것은 **등록 시점의
규칙**이다: 새 용어의 슬러그가 기존 슬러그와 충분히 비슷하면 새 URL을 만들지 않고
기존 것에 붙인다.

**자동 병합을 아직 넣지 않은 이유**: 표기가 비슷한 것과 개념이 같은 것은 다르다.
`agent`/`agents`는 같은 개념이지만 `transformer`(모델 구조)와 `transformers`
(허깅페이스 라이브러리)는 다르고, `open-weight`와 `open-weight-model`도 엄밀히는
방식과 대상이다. 잘못 합치면 **살아 있던 URL이 조용히 사라진다** — 색인된 페이지가
없어지는 쪽이 중복보다 손해가 크다.

**검토할 안**:
1. 규칙 기반(단수/복수, `-model(s)` 같은 접미어)으로 후보를 찾되 **자동 병합하지 않고
   경고만** 낸다. 사람이 별칭 파일에 확정한다. 가장 안전하고, 지금 구조에 바로 얹힌다.
2. 브리핑 작성 단계(SKILL.md)에서 기존 용어 목록을 먼저 보여주고 **같은 개념이면 기존
   `term_en` 표기를 그대로 쓰게** 한다. 원천에서 막는 방식이라 사후 수습이 필요 없지만,
   매일 실행에 목록 조회 단계가 하나 늘고 모델이 어길 수 있다.
3. 슬러그를 `term_en`이 아니라 고정 ID로 분리한다. 가장 튼튼하지만 이미 발행된 URL
   전부를 이전해야 해서, 지금 규모(75개)에서는 비용이 이득보다 크다.

현재 기울기는 **1안 + 2안 병행**이다. 결정 후 착수한다.
