...
### 2-0. 무엇으로 썼는지 기사마다 기록한다

기사마다 `sourced` 필드를 남긴다. **`"fulltext"`** = 원문을 읽고 썼다,
**`"rss_summary"`** = 원문을 끝내 못 읽어 피드 요약만으로 썼다.

왜 필요한가: "헤드라인이 아니라 원문을 읽는다"가 이 브리핑의 첫 번째 포지셔닝인데
그것만 숫자가 없다. 커뮤니티 카피에 "일부 매체는 원문 접근을 막아 피드 요약으로
대체된다"고 써 두고도 근거를 못 대는 상태다. `/data` 페이지가 이 값을 집계한다.

**기록만 한다.** 이 값은 선별에도 집필에도 영향을 주지 않는다 — 이미 끝난 작업을
사실대로 적는 것뿐이다. 부분적으로 읽은 경우(2-1의 2번, 리드 문단만 공개)는
`"fulltext"`로 센다. 요약에 필요한 사실을 원문에서 얻었다면 원문을 읽은 것이다.

값을 못 적겠으면 **키를 아예 빼라.** 추측해서 채우면 공개 지표가 거짓이 된다 —
없는 날은 집계에서 빠질 뿐 페이지는 정상 동작한다.
  ...
  "articles": [
    {
      ...
      "sourced": "fulltext | rss_summary — 2-0 참고. 확신이 없으면 키를 뺀다 (선택)",
      "link": "기사 URL",
      "source": "출처명",
...
rss_summary로 대체 작성한 기사 수(`sourced` 집계와 일치해야 한다), 커밋/푸시 결과, `docs/index.html` 경로, 이메일
